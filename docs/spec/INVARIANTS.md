...
### H6: BuildRef Integrity (Prepared Component Dependencies)

**Rule:** If `build.requiresBuilds` exists:
- `requiresBuilds[].itemId` must be unique within the build.
- No self-dependency: `requiresBuilds[].itemId !== build.itemId`.

**Rationale:**
- Prevents ambiguous dependency graphs and accidental recursion.
- Keeps cross-build composition auditable.

---

### H7: External Build References Must Be Declared

**Rule:** If a step consumes an external build output via `consumes[].source.type === "external_build"`, then the referenced `itemId` must exist in `build.requiresBuilds[].itemId`.

**Rationale:**
- Prevents hidden cross-build dependencies.
- Enables portfolio-level queries and validation.

---

### H8: In-Build Artifact References Must Resolve

**Rule:** If a step uses `consumes/produces` with `source.type === "in_build"`, then the referenced `artifactId` must exist in `build.artifacts[].id`.

**Rationale:**
- Prevents dangling flow edges.
- Ensures the material-flow graph is resolvable.
...
### S1: Kind/Family Coherence

**Rule:** If `step.kind` is set:
- If `step.kind === "quality_check"` then `step.action.family` should be `CHECK`.
- If `step.kind === "meta"` then `step.action.family` should be `OTHER`.

**Rationale:**
- Prevents two competing step-type systems.
- Keeps classification MECE (kind is a display hint; family is semantic spine).

---

### S2: Action/Ingredient Steps Should Have Target

**Rule:** For most steps, `step.target` should exist when the step is materially about a BOM item.

**Rationale:**
- Targets enable BOM tracing and bulk operations.
- 97.9% of legacy steps have extractable target names.

**Allowed exceptions:**
- Steps where target is implied by context
- Meta/admin instructions
- Explicit flow-only steps where semantics live in consumes/produces + notes
...
### S3: HEAT Steps Should Have Equipment
...
### S5: Published Steps Should Have Executable Instruction Text

**Rule:** If `build.status === "published"`, then each step should have non-empty `instruction`.

**Rationale:**
- SMEs need a reliable, human-executable instruction per step.
- We keep `notes` available as an escape hatch, but `notes` is not a substitute for clean instruction text.

---

### S6: Primary Output Artifact Should Be Set When Using Artifacts

**Rule:** If `build.artifacts` exists (non-empty), then `build.primaryOutputArtifactId` should exist and refer to a valid artifact.

**Rationale:**
- Establishes a stable default output for cross-build composition.
- Lets consumers refer to external build outputs without requiring multi-output modeling.

---

### S7: Phase Markers Should Match CookingPhase Field
...
### S8: Negation Patterns Should Use Exclude Flag
...
| Strong | 8 | S1, S2, S3, S4, S5, S6, S7, S8 |
| Soft | 3 | F1, F2, F3 |
| Deferred | 3 | D1, D2, D3 |
...
| Target coverage | % of steps with target when materially applicable | >95% |
| Equipment coverage | % of HEAT steps with equipment | >80% |
| Phase coverage | % of steps with cookingPhase | >50% |
| Instruction coverage | % of steps with instruction | track |
| Composition usage | % of builds with requiresBuilds | track |
| Artifact flow usage | % of builds with artifacts/primaryOutputArtifactId | track |
