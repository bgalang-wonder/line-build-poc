...
| --- | --- | --- | --- |
| `action` | ActionType | ✅ Yes | `PREP`, `HEAT`, `TRANSFER`, `COMBINE`, `ASSEMBLE`, `PORTION`, `CHECK`, `VEND`, `OTHER` |

> Note: Canonical action vocabulary lives in `docs/spec/SCHEMA-REFERENCE.md` (`ActionFamily`). Legacy terms like PLATE/FINISH should be represented via `VEND` + `CookingPhase.PASS` and/or `ASSEMBLE` with appropriate notes.
