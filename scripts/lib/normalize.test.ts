...
describe("normalize.ts", () => {
  it("auto-creates artifact stubs", () => {
    const build = baseBuild([
      step("s1", 1, ActionFamily.PREP, {
        output: [{ source: { type: "in_build", artifactId: "new-thing" } }]
      })
    ]);

    const normalized = normalizeBuild(build);
    expect(normalized.artifacts?.some(a => a.id === "new-thing")).toBe(true);
    expect(normalized.artifacts?.find(a => a.id === "new-thing")?.name).toBe("new-thing");
  });
  ...
  it("sets lineage and input roles for evolution + merge steps", () => {
    ...
    const normalized = normalizeBuild(build);
    const a2 = normalized.artifacts?.find((a) => a.id === "a2");
