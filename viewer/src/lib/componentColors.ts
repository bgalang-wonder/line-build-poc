/**
 * Component color utilities for the DAG visualization.
 * Assigns consistent colors to artifact groupIds for the Component Journey view.
 */

// Color palette for component groups - distinct, accessible colors
...
/**
 * Build a map of groupId -> color from the artifacts array.
 * Colors are assigned in alphabetical order of groupId for consistency.
 */
export function buildGroupColorMap(
  artifacts: Array<{ groupId?: string }>
): Map<string, string> {
  const uniqueGroups = Array.from(
    new Set(artifacts.map((a) => a.groupId).filter((g): g is string => Boolean(g)))
  ).sort();
...
/**
 * Get all unique groupIds from an artifacts array.
 */
export function getUniqueGroupIds(artifacts: Array<{ groupId?: string }>): string[] {
  return Array.from(
    new Set(artifacts.map((a) => a.groupId).filter((g): g is string => Boolean(g)))
  ).sort();
}
