...
export const FIXTURE_GRILLED_CHICKEN_BOWL: LineBuild = {
...
export const FIXTURE_CRISPY_FISH_TACOS: LineBuild = {
...
export const FIXTURE_BUDDHA_BOWL: LineBuild = {
...
export const FIXTURE_SIMPLE_SANDWICH: LineBuild = {
...
export const SAMPLE_FIXTURES: LineBuild[] = [
...
export const ACTIVE_FIXTURES = SAMPLE_FIXTURES.filter(
  (b) => b.metadata.status === 'active'
);

export const DRAFT_FIXTURES = SAMPLE_FIXTURES.filter(
  (b) => b.metadata.status === 'draft'
);
...
export function getFixtureById(buildId: string): LineBuild | undefined {
  return SAMPLE_FIXTURES.find((b) => b.id === buildId);
}
